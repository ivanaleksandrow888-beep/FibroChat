...
const INVITE_REQUESTS_FILE = path.join(DATA_DIR, "invite-requests.json");
...
const ATTACHMENTS_DIR = config.ATTACHMENTS_DIR;
const MAX_ATTACHMENT_BYTES = config.MAX_ATTACHMENT_BYTES;
const MAX_JSON_BODY_BYTES = config.MAX_JSON_BODY_BYTES;
...
const ADMIN_ACTIVITY_LIMIT = 250;
...
async function ensureDataStore() {
  ...
  for(const user of users){
    ...
    if(user.suspendedUntil===undefined){user.suspendedUntil=null;usersChanged=true;}
    if(user.securityNote===undefined){user.securityNote="";usersChanged=true;}
  }
...
const collectionByFile = new Map([
  [USERS_FILE,"users"],[INVITES_FILE,"invites"],[INVITE_REQUESTS_FILE,"invite_requests"],[MESSAGES_FILE,"messages"],[AUDIT_FILE,"audit"],
  [NOTIFICATIONS_FILE,"notifications"],[SUPPORT_FILE,"support"],[DEVICES_FILE,"devices"],
  [SESSIONS_FILE,"sessions"],[DEVICE_APPROVALS_FILE,"deviceApprovals"],[PUSH_SUBSCRIPTIONS_FILE,"pushSubscriptions"],[CONTACTS_FILE,"contacts"],[ATTACHMENTS_FILE,"attachments"],[GROUPS_FILE,"groups"],[GROUP_MESSAGES_FILE,"groupMessages"]
]);
...
function securityHeaders(){return{"X-Content-Type-Options":"nosniff","X-Frame-Options":"DENY","Referrer-Policy":"no-referrer","Permissions-Policy":"camera=(self), microphone=(self), geolocation=(), payment=(), usb=()","Cross-Origin-Resource-Policy":"same-origin"};}
function sendJson(res,statusCode,payload,traceId=crypto.randomUUID()){const value=(payload&&typeof payload==="object"&&!Array.isArray(payload))?{...payload,_protocol:protocolMeta(traceId)}:payload;const body=JSON.stringify(value);res.writeHead(statusCode,{"Content-Type":"application/json; charset=utf-8","Content-Length":Buffer.byteLength(body),"Cache-Control":"no-store",...securityHeaders(),"X-Fibro-Protocol":PROTOCOL_VERSION,"X-Fibro-Server-Version":APP_VERSION,"X-Trace-Id":traceId});res.end(body);}
...
function readBody(req){return new Promise((resolve,reject)=>{const declared=Number(req.headers["content-length"]||0);if(declared>MAX_JSON_BODY_BYTES){const error=new Error("BODY_TOO_LARGE");error.code="BODY_TOO_LARGE";return reject(error);}let body="";let size=0;let exceeded=false;req.on("data",c=>{if(exceeded)return;size+=Buffer.byteLength(c);if(size>MAX_JSON_BODY_BYTES){exceeded=true;body="";const error=new Error("BODY_TOO_LARGE");error.code="BODY_TOO_LARGE";reject(error);return;}body+=c;});req.on("end",()=>{if(exceeded)return;if(!body)return resolve({});try{resolve(JSON.parse(body));}catch{const error=new Error("INVALID_JSON");error.code="INVALID_JSON";reject(error);}});req.on("error",reject);});}
function readBinaryBody(req,maxBytes=MAX_ATTACHMENT_BYTES){return new Promise((resolve,reject)=>{const chunks=[];let size=0;req.on("data",chunk=>{size+=chunk.length;if(size>maxBytes){const error=new Error("ATTACHMENT_TOO_LARGE");error.code="ATTACHMENT_TOO_LARGE";reject(error);req.destroy();return;}chunks.push(chunk);});req.on("end",()=>resolve(Buffer.concat(chunks)));req.on("error",reject);});}
function safeAttachmentName(value){let name=String(value||"file").normalize("NFKC").replace(/[\/\\\0-\x1f\x7f]/g,"_").trim();if(!name)name="file";return name.slice(0,180);}
function attachmentForClient(item){return{id:item.id,ownerId:item.ownerId,recipientId:item.recipientId,name:item.name,mimeType:item.mimeType,size:item.originalSize,storedSize:item.storedSize,createdAt:item.createdAt};}
function mayAccessAttachment(item,userId){return item&&(item.ownerId===userId||item.recipientId===userId);}
...
function subscriptionState(user){ if(user.status==="suspended") return "suspended"; if(user.status!=="active") return "pending"; if(user.role==="super_admin") return "active"; if(!user.subscriptionEndsAt) return "expired"; const left=new Date(user.subscriptionEndsAt).getTime()-Date.now(); if(left<=0) return "expired"; if(left<=7*86400000) return "expiring"; return "active"; }
function subscriptionDaysRemaining(user){ if(user.role==="super_admin") return null; if(!user.subscriptionEndsAt) return 0; return Math.max(0, Math.ceil((new Date(user.subscriptionEndsAt).getTime()-Date.now())/86400000)); }
...
function publicMessage(message){return{id:message.id,senderId:message.senderId,recipientId:message.recipientId,envelope:message.envelope||null,signature:message.signature||null,createdAt:message.createdAt,editedAt:message.editedAt||null,deletedAt:message.deletedAt||null,deliveredAt:message.deliveredAt||null,readAt:message.readAt||null,deliveryAttempts:Number(message.deliveryAttempts)||0,lastAttemptAt:message.lastAttemptAt||null,nextAttemptAt:message.nextAttemptAt||null};}
...
function consumeRateLimit(bucketMap,key,limit,windowMs){const now=Date.now();let item=bucketMap.get(key);if(!item||now-item.startedAt>=windowMs)item={startedAt:now,count:0};item.count+=1;bucketMap.set(key,item);return{allowed:item.count<=limit,remaining:Math.max(0,limit-item.count),retryAfterMs:Math.max(0,windowMs-(now-item.startedAt))};}
function enforceApiRateLimit(req,res){if(req.method==="GET"&&req.url?.startsWith("/api/events"))return true;const key=clientIp(req);const result=consumeRateLimit(apiRateBuckets,key,config.API_RATE_MAX_REQUESTS,config.API_RATE_WINDOW_MS);if(result.allowed)return true;const retry=Math.max(1,Math.ceil(result.retryAfterMs/1000));res.setHeader("Retry-After",String(retry));sendJson(res,429,{ok:false,code:"RATE_LIMITED",error:"Слишком много запросов. Повторите через несколько секунд.",retryAfterSeconds:retry});return false;}
function enforcePublicInviteRateLimit(req,res){const key=clientIp(req);const result=consumeRateLimit(inviteRateBuckets,key,config.PUBLIC_INVITE_RATE_MAX_REQUESTS,config.PUBLIC_INVITE_RATE_WINDOW_MS);if(result.allowed)return true;const retry=Math.max(1,Math.ceil(result.retryAfterMs/1000));res.setHeader("Retry-After",String(retry));sendJson(res,429,{ok:false,code:"INVITE_RATE_LIMITED",error:"Слишком много запросов на приглашение. Попробуйте позже.",retryAfterSeconds:retry});return false;}
setInterval(()=>{const cutoff=Date.now()-Math.max(config.API_RATE_WINDOW_MS,config.PUBLIC_INVITE_RATE_WINDOW_MS)*2;for(const [key,item] of apiRateBuckets)if(item.startedAt<cutoff)apiRateBuckets.delete(key);for(const [key,item] of inviteRateBuckets)if(item.startedAt<cutoff)inviteRateBuckets.delete(key);},60000).unref();
...
function requireActive(req,res){const auth=requireAuth(req,res);if(!auth)return null;if(auth.user.status==="suspended"&&auth.user.suspendedUntil&&new Date(auth.user.suspendedUntil).getTime()<=Date.now()){const users=readJson(USERS_FILE);const user=users.find(x=>x.id===auth.user.id);if(user){user.status="active";user.suspendedUntil=null;writeJson(USERS_FILE,users);auth.user=user;audit("TEMP_SUSPENSION_EXPIRED",user.id,user.id,{})}}const expired=auth.user.role!=="super_admin"&&auth.user.subscriptionEndsAt&&new Date(auth.user.subscriptionEndsAt).getTime()<=Date.now();if(auth.user.status!=="active"||expired){sendJson(res,403,{ok:false,error:expired?"Срок подписки истёк":auth.user.status==="suspended"?"Аккаунт временно приостановлен":"Аккаунт ещё не подтверждён"});return null;}return auth;}
...
function canAdminManageTarget(actor,target){if(!actor||!target)return false;if(actor.role==="super_admin")return target.role!=="super_admin"||actor.id===target.id;return actor.role==="admin"&&target.role==="user";}
...
function serveStatic(res,pathname){const filePath=resolveClientFile(pathname);if(!filePath){res.writeHead(403,{"Content-Type":"text/plain; charset=utf-8"});return res.end("Доступ запрещён");}fs.stat(filePath,(error,stats)=>{if(error||!stats.isFile()){res.writeHead(404,{"Content-Type":"text/plain; charset=utf-8"});return res.end("Файл не найден");}const ext=path.extname(filePath).toLowerCase();res.writeHead(200,{"Content-Type":MIME_TYPES[ext]||"application/octet-stream","Cache-Control":[".html",".js",".css"].includes(ext)?"no-store":"public, max-age=300",...securityHeaders(),"Content-Security-Policy":"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self'; worker-src 'self' blob:; manifest-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'"});fs.createReadStream(filePath).pipe(res);});}
...
function publicInviteRequest(item){return{id:item.id,token:item.token,status:item.status,createdAt:item.createdAt,requestedAt:item.requestedAt||null,approvedAt:item.approvedAt||null,rejectedAt:item.rejectedAt||null,inviteCode:item.status==="approved"?item.inviteCode||null:null,expiresAt:item.expiresAt,requesterNickname:item.requesterNickname||null,requestedByNickname:item.requestedByNickname||null};}
function baseUrlForRequest(req){return (String(req.headers["x-forwarded-proto"]||"http").split(",")[0]+"://"+String(req.headers["x-forwarded-host"]||req.headers.host||"localhost")).replace(/\/$/,"");}
function createInviteRecord(createdBy,role="user",validDays=7,source="manual"){const invite={id:crypto.randomUUID(),code:`FIBRO-${crypto.randomBytes(4).toString("hex").toUpperCase()}`,role,createdBy,createdAt:new Date().toISOString(),expiresAt:new Date(Date.now()+validDays*86400000).toISOString(),usedAt:null,revokedAt:null,source};const invites=readJson(INVITES_FILE);invites.push(invite);writeJson(INVITES_FILE,invites);return invite;}
function getOrCreateSmartInvitation(req,user){const base=baseUrlForRequest(req);const now=Date.now();if(["admin","super_admin"].includes(user.role)){let invite=readJson(INVITES_FILE).find(i=>i.createdBy===user.id&&i.source==="profile_qr"&&!i.usedAt&&!i.revokedAt&&new Date(i.expiresAt).getTime()>now+3600000);if(!invite)invite=createInviteRecord(user.id,"user",7,"profile_qr");return{mode:"invite",url:`${base}/?invite=${encodeURIComponent(invite.code)}`,code:invite.code};}const requests=readJson(INVITE_REQUESTS_FILE);let link=requests.find(i=>i.kind==="referral_link"&&i.requestedBy===user.id&&i.status==="link"&&new Date(i.expiresAt).getTime()>now+3600000);if(!link){link={id:crypto.randomUUID(),token:crypto.randomBytes(18).toString("base64url"),kind:"referral_link",status:"link",requestedBy:user.id,requestedByNickname:user.nickname,createdAt:new Date().toISOString(),expiresAt:new Date(now+30*86400000).toISOString()};requests.push(link);writeJson(INVITE_REQUESTS_FILE,requests);}return{mode:"request",url:`${base}/?request=${encodeURIComponent(link.token)}`,token:link.token};}

async function handleApi(req,res,pathname,searchParams){
  if(!protocolCompatible(req,res))return true;
  if(!enforceApiRateLimit(req,res))return true;
  if(pathname==="/api/health"&&req.method==="GET"){const network=readObject(NETWORK_FILE,{});const users=readJson(USERS_FILE);sendJson(res,200,{ok:true,service:"FibroChat Head Node",version:APP_VERSION,nodeId:network.nodeId,networkId:network.networkId,networkName:network.networkName,bootstrapRequired:!users.some(x=>x.role==="super_admin"),protocolVersion:PROTOCOL_VERSION,minClientProtocol:MIN_CLIENT_PROTOCOL,capabilities:PROTOCOL_CAPABILITIES,encryption:"client-side",realtime:"sse-fetch-stream",deliveryQueue:"persistent-retry",database:store.mode,clusterReady:true,security:{rateLimit:true,maxJsonBodyBytes:MAX_JSON_BODY_BYTES,maxAttachmentBytes:MAX_ATTACHMENT_BYTES},time:new Date().toISOString()});return true;}
  ...
  if(pathname==="/api/invite-requests"&&req.method==="POST"){
    if(!enforcePublicInviteRateLimit(req,res))return true;
    const body=await readBody(req);const token=String(body.token||"").trim();const nickname=String(body.nickname||"").trim().slice(0,32);const requests=readJson(INVITE_REQUESTS_FILE);const link=requests.find(i=>i.token===token&&i.kind==="referral_link"&&i.status==="link"&&new Date(i.expiresAt).getTime()>Date.now());if(!link)return sendJson(res,404,{ok:false,error:"Ссылка запроса недействительна или устарела"}),true;
    let pending=requests.find(i=>i.parentToken===token&&i.status==="pending"&&Date.now()-new Date(i.createdAt).getTime()<86400000);if(!pending){pending={id:crypto.randomUUID(),token:crypto.randomBytes(18).toString("base64url"),parentToken:token,kind:"invite_request",status:"pending",requestedBy:link.requestedBy,requestedByNickname:link.requestedByNickname,requesterNickname:nickname||null,createdAt:new Date().toISOString(),requestedAt:new Date().toISOString(),expiresAt:new Date(Date.now()+24*3600000).toISOString()};requests.push(pending);writeJson(INVITE_REQUESTS_FILE,requests);for(const admin of readJson(USERS_FILE).filter(u=>["admin","super_admin"].includes(u.role)))notify(admin.id,"INVITE_REQUESTED","Запрошено приглашение",`${link.requestedByNickname||"Пользователь"} пригласил нового участника${nickname?` «${nickname}»`:""}. Подтвердите выдачу инвайта.`,{requestId:pending.id});audit("INVITE_REQUESTED",link.requestedBy,pending.id,{requesterNickname:nickname||null});}sendJson(res,201,{ok:true,request:publicInviteRequest(pending)});return true;
  }
  const publicRequestMatch=pathname.match(/^\/api\/invite-requests\/([A-Za-z0-9_-]+)$/);if(publicRequestMatch&&req.method==="GET"){const item=readJson(INVITE_REQUESTS_FILE).find(i=>i.token===publicRequestMatch[1]&&i.kind==="invite_request");if(!item)return sendJson(res,404,{ok:false,error:"Запрос не найден"}),true;sendJson(res,200,{ok:true,request:publicInviteRequest(item)});return true;}
  ...
  if(pathname==="/api/login"&&req.method==="POST"){
    ...
    if(user.status==="suspended"){if(user.suspendedUntil&&new Date(user.suspendedUntil).getTime()<=Date.now()){const users=readJson(USERS_FILE);const stored=users.find(x=>x.id===user.id);stored.status="active";stored.suspendedUntil=null;writeJson(USERS_FILE,users);audit("TEMP_SUSPENSION_EXPIRED",user.id,user.id,{});}else{return sendJson(res,403,{ok:false,code:"ACCOUNT_SUSPENDED",error:user.suspendedUntil?`Аккаунт приостановлен до ${new Date(user.suspendedUntil).toLocaleString("ru-RU")}`:"Аккаунт приостановлен администратором"}),true;}}
  ...
  if(pathname==="/api/profile"&&req.method==="GET"){const auth=requireAuth(req,res);if(!auth)return true;const smartInvitation=getOrCreateSmartInvitation(req,auth.user);const qrDataUrl=await QRCode.toDataURL(smartInvitation.url,{width:320,margin:2,errorCorrectionLevel:"M"});sendJson(res,200,{ok:true,profile:publicUser(auth.user),inviteUrl:smartInvitation.url,inviteMode:smartInvitation.mode,inviteCode:smartInvitation.code||null,qrDataUrl});return true;}
  ...
  if(pathname==="/api/attachments"&&req.method==="POST"){const auth=requireActive(req,res);if(!auth)return true;const recipientId=String(searchParams.get("recipientId")||"");if(!canContact(auth.user,recipientId))return sendJson(res,403,{ok:false,error:"Получатель не входит в ваши контакты"}),true;const recipient=readJson(USERS_FILE).find(u=>u.id===recipientId&&u.status==="active");if(!recipient)return sendJson(res,404,{ok:false,error:"Получатель недоступен"}),true;const declaredLength=Number(req.headers["content-length"]||0);if(declaredLength>MAX_ATTACHMENT_BYTES)return sendJson(res,413,{ok:false,code:"ATTACHMENT_TOO_LARGE",error:`Вложение превышает лимит ${Math.floor(MAX_ATTACHMENT_BYTES/1024/1024)} МБ`}),true;let payload;try{payload=await readBinaryBody(req);}catch(error){if(error.code==="ATTACHMENT_TOO_LARGE")return sendJson(res,413,{ok:false,code:error.code,error:`Вложение превышает лимит ${Math.floor(MAX_ATTACHMENT_BYTES/1024/1024)} МБ`}),true;throw error;}if(!payload.length)return sendJson(res,400,{ok:false,error:"Пустое вложение"}),true;const id=crypto.randomUUID();const item={id,ownerId:auth.user.id,recipientId,name:safeAttachmentName(decodeURIComponent(String(req.headers["x-file-name"]||"file"))),mimeType:String(req.headers["content-type"]||"application/octet-stream").slice(0,120),originalSize:Math.max(0,Number(req.headers["x-original-size"]||payload.length)),storedSize:payload.length,sha256:crypto.createHash("sha256").update(payload).digest("hex"),createdAt:new Date().toISOString(),storage:"opaque-binary-v1"};fs.writeFileSync(path.join(ATTACHMENTS_DIR,`${id}.bin`),payload,{flag:"wx",mode:0o600});const list=readJson(ATTACHMENTS_FILE);list.push(item);writeJson(ATTACHMENTS_FILE,list);audit("ATTACHMENT_UPLOADED",auth.user.id,recipientId,{attachmentId:id,size:item.storedSize,mimeType:item.mimeType});sendJson(res,201,{ok:true,attachment:attachmentForClient(item),maxBytes:MAX_ATTACHMENT_BYTES});return true;}
  const attachmentMatch=pathname.match(/^\/api\/attachments\/([0-9a-f-]+)$/i);if(attachmentMatch&&req.method==="GET"){const auth=requireActive(req,res);if(!auth)return true;const item=readJson(ATTACHMENTS_FILE).find(x=>x.id===attachmentMatch[1]);if(!item||!mayAccessAttachment(item,auth.user.id))return sendJson(res,404,{ok:false,error:"Вложение не найдено"}),true;const filePath=path.join(ATTACHMENTS_DIR,`${item.id}.bin`);let stat;try{stat=fs.statSync(filePath);}catch{return sendJson(res,410,{ok:false,error:"Файл вложения отсутствует в хранилище"}),true;}res.writeHead(200,{"Content-Type":"application/octet-stream","Content-Length":stat.size,"Content-Disposition":`attachment; filename*=UTF-8''${encodeURIComponent(item.name)}`,"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff","X-Fibro-Attachment-Name":encodeURIComponent(item.name),"X-Fibro-Attachment-Type":item.mimeType,"X-Fibro-Attachment-Size":String(item.originalSize)});fs.createReadStream(filePath).pipe(res);return true;}

  if(pathname==="/api/groups"&&req.method==="GET"){
    const auth=requireActive(req,res);if(!auth)return true;
    const users=readJson(USERS_FILE);const groups=readJson(GROUPS_FILE).filter(g=>Array.isArray(g.members)&&g.members.some(m=>m.userId===auth.user.id));
    const result=groups.map(g=>({id:g.id,name:g.name,description:g.description||"",avatarDataUrl:g.avatarDataUrl||"",ownerId:g.ownerId,createdAt:g.createdAt,updatedAt:g.updatedAt,members:g.members.map(m=>{const u=users.find(x=>x.id===m.userId);return{userId:m.userId,role:m.role,nickname:u?.nickname||"Удалённый пользователь",displayName:u?.displayName||u?.nickname||"Удалённый пользователь",avatarDataUrl:u?.avatarDataUrl||""};}),memberCount:g.members.length}));
    sendJson(res,200,{ok:true,groups:result});return true;
  }
  if(pathname==="/api/groups"&&req.method==="POST"){
    const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);
    const name=String(body.name||"").trim().slice(0,80);const description=String(body.description||"").trim().slice(0,500);const requested=Array.isArray(body.memberIds)?body.memberIds.map(String):[];
    if(name.length<2)return sendJson(res,400,{ok:false,error:"Название группы должно содержать минимум 2 символа"}),true;
    const users=readJson(USERS_FILE);const allowed=new Set(readJson(CONTACTS_FILE).filter(c=>c.userId===auth.user.id).map(c=>c.contactUserId));
    const ids=[...new Set([auth.user.id,...requested.filter(id=>allowed.has(id))])];if(ids.length<2)return sendJson(res,400,{ok:false,error:"Добавьте хотя бы одного участника из контактов"}),true;
    if(ids.length>50)return sendJson(res,400,{ok:false,error:"В группе может быть не более 50 участников"}),true;
    const now=new Date().toISOString();const group={id:crypto.randomUUID(),name,description,avatarDataUrl:"",ownerId:auth.user.id,createdAt:now,updatedAt:now,members:ids.map(id=>({userId:id,role:id===auth.user.id?"owner":"member",joinedAt:now}))};
    const groups=readJson(GROUPS_FILE);groups.push(group);writeJson(GROUPS_FILE,groups);for(const id of ids.filter(id=>id!==auth.user.id)){notify(id,"GROUP_INVITE",`Вас добавили в группу «${name}»`,`${auth.user.displayName||auth.user.nickname} создал группу`,{groupId:group.id});sendEvent(id,"group:updated",{groupId:group.id});}
    audit("GROUP_CREATED",auth.user.id,group.id,{name,memberCount:ids.length});sendJson(res,201,{ok:true,group});return true;
  }
  const groupMatch=pathname.match(/^\/api\/groups\/([0-9a-f-]+)$/i);
  if(groupMatch&&req.method==="GET"){
    const auth=requireActive(req,res);if(!auth)return true;const group=readJson(GROUPS_FILE).find(g=>g.id===groupMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const users=readJson(USERS_FILE);sendJson(res,200,{ok:true,group:{...group,members:group.members.map(m=>({userId:m.userId,role:m.role,joinedAt:m.joinedAt,user:publicUser(users.find(u=>u.id===m.userId)||{id:m.userId,nickname:"Удалённый пользователь",status:"inactive"})}))}});return true;
  }
  if(groupMatch&&req.method==="PATCH"){
    const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);const groups=readJson(GROUPS_FILE);const group=groups.find(g=>g.id===groupMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const actor=group.members.find(m=>m.userId===auth.user.id);if(!["owner","admin"].includes(actor.role))return sendJson(res,403,{ok:false,error:"Недостаточно прав"}),true;
    if(typeof body.name==="string"){const name=body.name.trim().slice(0,80);if(name.length<2)return sendJson(res,400,{ok:false,error:"Слишком короткое название"}),true;group.name=name;}
    if(typeof body.description==="string")group.description=body.description.trim().slice(0,500);
    group.updatedAt=new Date().toISOString();writeJson(GROUPS_FILE,groups);for(const m of group.members)sendEvent(m.userId,"group:updated",{groupId:group.id});audit("GROUP_UPDATED",auth.user.id,group.id,{});sendJson(res,200,{ok:true,group});return true;
  }
  if(groupMatch&&req.method==="DELETE"){
    const auth=requireActive(req,res);if(!auth)return true;const groups=readJson(GROUPS_FILE);const index=groups.findIndex(g=>g.id===groupMatch[1]&&g.ownerId===auth.user.id);if(index<0)return sendJson(res,404,{ok:false,error:"Группа не найдена или вы не владелец"}),true;
    const [group]=groups.splice(index,1);writeJson(GROUPS_FILE,groups);const messages=readJson(GROUP_MESSAGES_FILE).filter(m=>m.groupId!==group.id);writeJson(GROUP_MESSAGES_FILE,messages);for(const m of group.members)sendEvent(m.userId,"group:removed",{groupId:group.id});audit("GROUP_DELETED",auth.user.id,group.id,{name:group.name});sendJson(res,200,{ok:true});return true;
  }
  const groupMembersCollectionMatch=pathname.match(/^\/api\/groups\/([0-9a-f-]+)\/members$/i);
  if(groupMembersCollectionMatch&&req.method==="POST"){
    const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);const groups=readJson(GROUPS_FILE);const group=groups.find(g=>g.id===groupMembersCollectionMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const actor=group.members.find(m=>m.userId===auth.user.id);if(!["owner","admin"].includes(actor.role))return sendJson(res,403,{ok:false,error:"Недостаточно прав"}),true;const userId=String(body.userId||"");if(group.members.some(m=>m.userId===userId))return sendJson(res,409,{ok:false,error:"Пользователь уже в группе"}),true;if(group.members.length>=50)return sendJson(res,400,{ok:false,error:"В группе может быть не более 50 участников"}),true;
    const allowed=readJson(CONTACTS_FILE).some(c=>c.userId===auth.user.id&&c.contactUserId===userId);const user=readJson(USERS_FILE).find(u=>u.id===userId&&u.status==="active"&&u.encryptionPublicKey&&u.signingPublicKey);if(!allowed||!user)return sendJson(res,403,{ok:false,error:"Можно добавлять только активные контакты с настроенными ключами"}),true;
    const now=new Date().toISOString();group.members.push({userId,role:"member",joinedAt:now});group.updatedAt=now;writeJson(GROUPS_FILE,groups);for(const m of group.members)sendEvent(m.userId,"group:updated",{groupId:group.id});notify(userId,"GROUP_INVITE",`Вас добавили в группу «${group.name}»`,`${auth.user.displayName||auth.user.nickname} добавил вас в группу`,{groupId:group.id});audit("GROUP_MEMBER_ADDED",auth.user.id,userId,{groupId:group.id});sendJson(res,201,{ok:true});return true;
  }
  const memberMatch=pathname.match(/^\/api\/groups\/([0-9a-f-]+)\/members\/([0-9a-f-]+)$/i);
  if(memberMatch&&req.method==="PATCH"){
    const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);const groups=readJson(GROUPS_FILE);const group=groups.find(g=>g.id===memberMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const actor=group.members.find(m=>m.userId===auth.user.id);if(actor.role!=="owner")return sendJson(res,403,{ok:false,error:"Только владелец может менять роли"}),true;const target=group.members.find(m=>m.userId===memberMatch[2]);if(!target||target.role==="owner")return sendJson(res,400,{ok:false,error:"Роль изменить нельзя"}),true;target.role=body.role==="admin"?"admin":"member";group.updatedAt=new Date().toISOString();writeJson(GROUPS_FILE,groups);for(const m of group.members)sendEvent(m.userId,"group:updated",{groupId:group.id});sendJson(res,200,{ok:true});return true;
  }
  if(memberMatch&&req.method==="DELETE"){
    const auth=requireActive(req,res);if(!auth)return true;const groups=readJson(GROUPS_FILE);const group=groups.find(g=>g.id===memberMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;const actor=group.members.find(m=>m.userId===auth.user.id);const target=group.members.find(m=>m.userId===memberMatch[2]);if(!target)return sendJson(res,404,{ok:false,error:"Участник не найден"}),true;
    if(memberMatch[2]!==auth.user.id&&!["owner","admin"].includes(actor.role))return sendJson(res,403,{ok:false,error:"Недостаточно прав"}),true;if(target.role==="owner")return sendJson(res,400,{ok:false,error:"Владелец не может покинуть группу без передачи прав"}),true;group.members=group.members.filter(m=>m.userId!==memberMatch[2]);group.updatedAt=new Date().toISOString();writeJson(GROUPS_FILE,groups);for(const m of group.members)sendEvent(m.userId,"group:updated",{groupId:group.id});sendEvent(memberMatch[2],"group:removed",{groupId:group.id});sendJson(res,200,{ok:true});return true;
  }
  const groupMessagesMatch=pathname.match(/^\/api\/groups\/([0-9a-f-]+)\/messages$/i);
  if(groupMessagesMatch&&req.method==="GET"){
    const auth=requireActive(req,res);if(!auth)return true;const group=readJson(GROUPS_FILE).find(g=>g.id===groupMessagesMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const messages=readJson(GROUP_MESSAGES_FILE).filter(m=>m.groupId===group.id).map(m=>{const delivery=(m.deliveries||[]).find(d=>d.recipientId===auth.user.id)||((m.senderId===auth.user.id)?(m.deliveries||[])[0]:null);return delivery?{id:m.id,groupId:m.groupId,senderId:m.senderId,recipientId:delivery.recipientId,envelope:delivery.envelope,signature:delivery.signature,createdAt:m.createdAt}:null;}).filter(Boolean);sendJson(res,200,{ok:true,messages});return true;
  }
  if(groupMessagesMatch&&req.method==="POST"){
    const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);const group=readJson(GROUPS_FILE).find(g=>g.id===groupMessagesMatch[1]&&g.members?.some(m=>m.userId===auth.user.id));if(!group)return sendJson(res,404,{ok:false,error:"Группа не найдена"}),true;
    const deliveries=Array.isArray(body.deliveries)?body.deliveries:[];const memberIds=new Set(group.members.map(m=>m.userId));if(deliveries.length!==memberIds.size)return sendJson(res,400,{ok:false,error:"Зашифрованная копия нужна для каждого участника"}),true;let messageId="",createdAt="";const seen=new Set();for(const d of deliveries){const recipientId=String(d.recipientId||"");if(!memberIds.has(recipientId)||seen.has(recipientId)||!validEnvelope(d.envelope,auth.user.id,recipientId)||typeof d.signature!=="string")return sendJson(res,400,{ok:false,error:"Некорректный групповой пакет"}),true;const ok=await verifyEnvelopeSignature(d.envelope,d.signature,auth.user.signingPublicKey);if(!ok)return sendJson(res,400,{ok:false,error:"Подпись группового сообщения не прошла проверку"}),true;if(!messageId){messageId=d.envelope.messageId;createdAt=d.envelope.createdAt;}if(d.envelope.messageId!==messageId||d.envelope.createdAt!==createdAt)return sendJson(res,400,{ok:false,error:"Копии сообщения не совпадают"}),true;seen.add(recipientId);}
    const all=readJson(GROUP_MESSAGES_FILE);if(all.some(m=>m.id===messageId))return sendJson(res,409,{ok:false,error:"Дубликат сообщения"}),true;all.push({id:messageId,groupId:group.id,senderId:auth.user.id,createdAt,deliveries});writeJson(GROUP_MESSAGES_FILE,all);for(const m of group.members.filter(m=>m.userId!==auth.user.id)){sendEvent(m.userId,"group:message",{groupId:group.id,messageId});void sendWebPush(m.userId,{title:`${group.name}: ${auth.user.nickname}`,body:"Новое защищённое сообщение",tag:`group-${messageId}`,url:"/"});}sendJson(res,201,{ok:true,messageId});return true;
  }
  ...
  const updateMessageMatch=pathname.match(/^\/api\/messages\/([0-9a-f-]+)$/i);if(updateMessageMatch&&req.method==="PUT"){const auth=requireActive(req,res);if(!auth)return true;const body=await readBody(req);const messages=readJson(MESSAGES_FILE);const message=messages.find(x=>x.id===updateMessageMatch[1]&&x.senderId===auth.user.id);if(!message||message.deletedAt)return sendJson(res,404,{ok:false,error:"Сообщение не найдено"}),true;if(!validEnvelope(body.envelope,auth.user.id,message.recipientId)||body.envelope.messageId!==message.id||typeof body.signature!=="string")return sendJson(res,400,{ok:false,error:"Некорректный пакет редактирования"}),true;const verified=await verifyEnvelopeSignature(body.envelope,body.signature,auth.user.signingPublicKey);if(!verified)return sendJson(res,400,{ok:false,error:"Подпись изменения не прошла проверку"}),true;message.envelope=body.envelope;message.signature=body.signature;message.editedAt=new Date().toISOString();writeJson(MESSAGES_FILE,messages);sendEvent(message.recipientId,"message:updated",{messageId:message.id,editedAt:message.editedAt,senderId:message.senderId});sendEvent(message.senderId,"message:updated",{messageId:message.id,editedAt:message.editedAt,senderId:message.senderId});audit("MESSAGE_EDITED",auth.user.id,message.recipientId,{messageId:message.id});sendJson(res,200,{ok:true,message:publicMessage(message)});return true;}
  if(updateMessageMatch&&req.method==="DELETE"){const auth=requireActive(req,res);if(!auth)return true;const messages=readJson(MESSAGES_FILE);const message=messages.find(x=>x.id===updateMessageMatch[1]&&x.senderId===auth.user.id);if(!message)return sendJson(res,404,{ok:false,error:"Сообщение не найдено"}),true;if(!message.deletedAt){message.deletedAt=new Date().toISOString();message.editedAt=null;writeJson(MESSAGES_FILE,messages);sendEvent(message.recipientId,"message:deleted",{messageId:message.id,deletedAt:message.deletedAt,senderId:message.senderId});sendEvent(message.senderId,"message:deleted",{messageId:message.id,deletedAt:message.deletedAt,senderId:message.senderId});audit("MESSAGE_DELETED",auth.user.id,message.recipientId,{messageId:message.id});}sendJson(res,200,{ok:true,message:publicMessage(message)});return true;}
  ...
  if(pathname==="/api/admin/diagnostics"&&req.method==="GET"){
    ...
    const network=readObject(NETWORK_FILE,{});const users=readJson(USERS_FILE);const sessions=readJson(SESSIONS_FILE);const push=readJson(PUSH_SUBSCRIPTIONS_FILE);const attachments=readJson(ATTACHMENTS_FILE);const auditEvents=readJson(AUDIT_FILE);const storage=await store.diagnostics();
    const activeSessions=sessions.filter(x=>!x.revokedAt&&new Date(x.expiresAt).getTime()>Date.now()).length;
    const lastSecurityEvents=auditEvents.filter(e=>/LOGIN_|DEVICE_|SUSPEND|ROLE_|INVITE_|SESSIONS_REVOKED/.test(e.type)).slice(-12).reverse().map(e=>({type:e.type,createdAt:e.createdAt,actorId:e.actorId,targetId:e.targetId}));
    let attachmentBytes=0;for(const item of attachments)attachmentBytes+=Number(item.storedSize)||0;
    const diagnostics={version:APP_VERSION,protocolVersion:PROTOCOL_VERSION,uptimeSeconds:Math.floor((Date.now()-serverStartedAt)/1000),nodeId:network.nodeId||null,networkId:network.networkId||null,storage,users:{total:users.length,active:users.filter(u=>subscriptionState(u)==="active").length,suspended:users.filter(u=>u.status==="suspended").length},sessions:{active:activeSessions,known:sessions.length},realtime:{onlineUsers:[...presence.keys()].filter(isOnline).length,sseUsers:eventClients.size,sseConnections:[...eventClients.values()].reduce((sum,set)=>sum+set.size,0)},push:{configured:Boolean(network.webPushVapid?.publicKey&&network.webPushVapid?.privateKey),subscriptions:push.length},turn:{configured:Boolean(config.TURN_URLS.length&&(config.TURN_SHARED_SECRET||(config.TURN_USERNAME&&config.TURN_CREDENTIAL))),mode:config.TURN_SHARED_SECRET?"temporary":config.TURN_USERNAME&&config.TURN_CREDENTIAL?"static":"disabled",urls:config.TURN_URLS.map(url=>String(url).replace(/([?&](?:credential|password)=)[^&]+/gi,"$1***"))},limits:{jsonBodyBytes:MAX_JSON_BODY_BYTES,attachmentBytes:MAX_ATTACHMENT_BYTES,apiRequests:config.API_RATE_MAX_REQUESTS,apiWindowMs:config.API_RATE_WINDOW_MS},attachments:{count:attachments.length,storedBytes:attachmentBytes},recentSecurityEvents:lastSecurityEvents,recentErrors:recentServerErrors.slice(-12).reverse()};
    sendJson(res,200,{ok:true,diagnostics});return true;
  }
  if(pathname==="/api/admin/dashboard"&&req.method==="GET"){
    const auth=requireAdmin(req,res);if(!auth)return true;
    const users=readJson(USERS_FILE);const invites=readJson(INVITES_FILE);const network=readObject(NETWORK_FILE,{});
    const summary={totalUsers:users.length,activeUsers:users.filter(u=>subscriptionState(u)==="active").length,pendingUsers:users.filter(u=>u.status==="pending").length,expiringUsers:users.filter(u=>subscriptionState(u)==="expiring").length,expiredUsers:users.filter(u=>subscriptionState(u)==="expired").length,activeInvites:invites.filter(i=>!i.usedAt&&!i.revokedAt&&new Date(i.expiresAt).getTime()>Date.now()).length,suspendedUsers:users.filter(u=>u.status==="suspended").length,activeSessions:readJson(SESSIONS_FILE).filter(s=>!s.revokedAt&&new Date(s.expiresAt).getTime()>Date.now()).length,failedLogins24h:readJson(AUDIT_FILE).filter(e=>["LOGIN_FAILED","LOGIN_BLOCKED","LOGIN_RATE_LIMITED"].includes(e.type)&&Date.now()-new Date(e.createdAt).getTime()<86400000).length};
  ...
  if(pathname==="/api/admin/users"&&req.method==="GET"){if(!requireAdmin(req,res))return true;const query=String(searchParams.get("q")||"").trim().toLowerCase();const status=String(searchParams.get("status")||"");const role=String(searchParams.get("role")||"");let users=readJson(USERS_FILE);if(query)users=users.filter(u=>[u.nickname,u.displayName,u.fibroId].some(v=>String(v||"").toLowerCase().includes(query)));if(status)users=users.filter(u=>u.status===status);if(role)users=users.filter(u=>u.role===role);sendJson(res,200,{ok:true,users:users.map(publicUser)});return true;}
  if(pathname==="/api/admin/audit"&&req.method==="GET"){if(!requireAdmin(req,res))return true;const type=String(searchParams.get("type")||"");let events=readJson(AUDIT_FILE);if(type)events=events.filter(e=>e.type===type);events=events.slice(-ADMIN_ACTIVITY_LIMIT).reverse();sendJson(res,200,{ok:true,events});return true;}
  if(pathname==="/api/admin/security/activity"&&req.method==="GET"){if(!requireAdmin(req,res))return true;const securityTypes=new Set(["LOGIN_SUCCESS","LOGIN_FAILED","LOGIN_BLOCKED","LOGIN_RATE_LIMITED","REFRESH_REJECTED","DEVICE_PENDING","DEVICE_TRUSTED","DEVICE_REVOKED","ALL_SESSIONS_REVOKED","ADMIN_SESSIONS_REVOKED","USER_SUSPENDED","USER_TEMP_SUSPENDED","USER_RESTORED","ROLE_CHANGED"]);const events=readJson(AUDIT_FILE).filter(e=>securityTypes.has(e.type)).slice(-ADMIN_ACTIVITY_LIMIT).reverse();sendJson(res,200,{ok:true,events});return true;}
  if(pathname==="/api/admin/invites"&&req.method==="GET"){if(!requireAdmin(req,res))return true;const invites=readJson(INVITES_FILE).slice().reverse();sendJson(res,200,{ok:true,invites});return true;}
  if(pathname==="/api/admin/invite-requests"&&req.method==="GET"){if(!requireAdmin(req,res))return true;const requests=readJson(INVITE_REQUESTS_FILE).filter(i=>i.kind==="invite_request").slice().reverse().map(publicInviteRequest);sendJson(res,200,{ok:true,requests});return true;}
  const inviteRequestAction=pathname.match(/^\/api\/admin\/invite-requests\/([0-9a-f-]+)\/(approve|reject)$/i);if(inviteRequestAction&&req.method==="POST"){const auth=requireAdmin(req,res);if(!auth)return true;const requests=readJson(INVITE_REQUESTS_FILE);const item=requests.find(i=>i.id===inviteRequestAction[1]&&i.kind==="invite_request");if(!item)return sendJson(res,404,{ok:false,error:"Запрос не найден"}),true;if(item.status!=="pending")return sendJson(res,409,{ok:false,error:"Запрос уже обработан"}),true;if(inviteRequestAction[2]==="approve"){const invite=createInviteRecord(auth.user.id,"user",7,"approved_request");item.status="approved";item.inviteCode=invite.code;item.approvedAt=new Date().toISOString();item.approvedBy=auth.user.id;audit("INVITE_REQUEST_APPROVED",auth.user.id,item.id,{inviteId:invite.id});if(item.requestedBy)notify(item.requestedBy,"INVITE_REQUEST_APPROVED","Приглашение одобрено",`Администратор выдал код ${invite.code}.`,{requestId:item.id,inviteCode:invite.code});}else{item.status="rejected";item.rejectedAt=new Date().toISOString();item.rejectedBy=auth.user.id;audit("INVITE_REQUEST_REJECTED",auth.user.id,item.id,{});if(item.requestedBy)notify(item.requestedBy,"INVITE_REQUEST_REJECTED","Запрос отклонён","Администратор отклонил запрос на приглашение.",{requestId:item.id});}writeJson(INVITE_REQUESTS_FILE,requests);sendJson(res,200,{ok:true,request:publicInviteRequest(item)});return true;}
  if(pathname==="/api/admin/invites"&&req.method==="POST"){
    ...
    const role=["user","admin"].includes(String(body.role||""))?String(body.role):"user";if(role==="admin"&&auth.user.role!=="super_admin")return sendJson(res,403,{ok:false,error:"Только головной администратор может создавать admin-инвайты"}),true;const invite=createInviteRecord(auth.user.id,role,validDays,"manual");
    audit("INVITE_CREATED",auth.user.id,invite.id,{validDays,role});sendJson(res,201,{ok:true,invite});return true;
  }
  ...
  if(extendMatch&&req.method==="POST"){
    const auth=requireHead(req,res);if(!auth)return true;const body=await readBody(req);const days=Math.max(1,Math.min(365,Number(body.days)||30));const users=readJson(USERS_FILE);const user=users.find(x=>x.id===extendMatch[1]);if(!user)return sendJson(res,404,{ok:false,error:"Пользователь не найден"}),true;if(user.role==="super_admin")return sendJson(res,409,{ok:false,error:"Подписка суперадминистратора бессрочная и не требует продления"}),true;
  ...
  if(suspendMatch&&req.method==="POST"){
    const auth=requireAdmin(req,res);if(!auth)return true;const users=readJson(USERS_FILE);const user=users.find(x=>x.id===suspendMatch[1]);if(!user)return sendJson(res,404,{ok:false,error:"Пользователь не найден"}),true;if(user.role==="super_admin")return sendJson(res,403,{ok:false,error:"Головной аккаунт нельзя приостановить"}),true;if(!canAdminManageTarget(auth.user,user))return sendJson(res,403,{ok:false,error:"Администратор может управлять только обычными пользователями"}),true;
    user.status="suspended";user.suspendedUntil=null;writeJson(USERS_FILE,users);revokeUserSessions(user.id,"admin_suspension");presence.delete(user.id);notify(user.id,"ACCESS_SUSPENDED","Доступ приостановлен","Администратор приостановил сетевой доступ.",{});audit("USER_SUSPENDED",auth.user.id,user.id,{});sendJson(res,200,{ok:true,user:publicUser(user)});return true;
  }
  ...
  const tempSuspendMatch=pathname.match(/^\/api\/admin\/users\/([0-9a-f-]+)\/suspend-temporary$/i);
  if(tempSuspendMatch&&req.method==="POST"){const auth=requireAdmin(req,res);if(!auth)return true;const body=await readBody(req);const minutes=Math.max(5,Math.min(43200,Number(body.minutes)||60));const users=readJson(USERS_FILE);const user=users.find(x=>x.id===tempSuspendMatch[1]);if(!user)return sendJson(res,404,{ok:false,error:"Пользователь не найден"}),true;if(user.role==="super_admin")return sendJson(res,403,{ok:false,error:"Головной аккаунт нельзя приостановить"}),true;if(!canAdminManageTarget(auth.user,user))return sendJson(res,403,{ok:false,error:"Администратор может управлять только обычными пользователями"}),true;user.status="suspended";user.suspendedUntil=new Date(Date.now()+minutes*60000).toISOString();user.securityNote=String(body.reason||"").trim().slice(0,240);writeJson(USERS_FILE,users);revokeUserSessions(user.id,"temporary_suspension");presence.delete(user.id);audit("USER_TEMP_SUSPENDED",auth.user.id,user.id,{minutes,reason:user.securityNote,until:user.suspendedUntil});notify(user.id,"ACCESS_TEMP_SUSPENDED","Доступ временно приостановлен",`Доступ приостановлен до ${new Date(user.suspendedUntil).toLocaleString("ru-RU")}.`,{until:user.suspendedUntil});sendJson(res,200,{ok:true,user:publicUser(user)});return true;}
  const revokeSessionsMatch=pathname.match(/^\/api\/admin\/users\/([0-9a-f-]+)\/sessions\/revoke$/i);
  if(revokeSessionsMatch&&req.method==="POST"){const auth=requireAdmin(req,res);if(!auth)return true;const target=readJson(USERS_FILE).find(x=>x.id===revokeSessionsMatch[1]);if(!target)return sendJson(res,404,{ok:false,error:"Пользователь не найден"}),true;if(target.role==="super_admin"&&auth.user.id!==target.id)return sendJson(res,403,{ok:false,error:"Нельзя завершить сессии головного администратора"}),true;if(target.id!==auth.user.id&&!canAdminManageTarget(auth.user,target))return sendJson(res,403,{ok:false,error:"Администратор может завершать сессии только обычных пользователей"}),true;revokeUserSessions(target.id,"admin_revoked");presence.delete(target.id);audit("ADMIN_SESSIONS_REVOKED",auth.user.id,target.id,{});notify(target.id,"SESSIONS_REVOKED","Сессии завершены","Администратор завершил все активные сессии аккаунта.",{});sendJson(res,200,{ok:true});return true;}
  const inviteRevokeMatch=pathname.match(/^\/api\/admin\/invites\/([0-9a-f-]+)\/revoke$/i);
  if(inviteRevokeMatch&&req.method==="POST"){const auth=requireAdmin(req,res);if(!auth)return true;const invites=readJson(INVITES_FILE);const invite=invites.find(x=>x.id===inviteRevokeMatch[1]);if(!invite)return sendJson(res,404,{ok:false,error:"Инвайт не найден"}),true;if(invite.usedAt)return sendJson(res,409,{ok:false,error:"Использованный инвайт нельзя отозвать"}),true;invite.revokedAt=new Date().toISOString();invite.revokedBy=auth.user.id;writeJson(INVITES_FILE,invites);audit("INVITE_REVOKED",auth.user.id,invite.id,{});sendJson(res,200,{ok:true});return true;}
  return false;
}
function createServer(){return http.createServer(async(req,res)=>{const traceId=crypto.randomUUID();const started=Date.now();try{const url=new URL(req.url,`http://${req.headers.host||"localhost"}`);if(url.pathname.startsWith("/api/")){const handled=await handleApi(req,res,url.pathname,url.searchParams);if(!handled)sendJson(res,404,{ok:false,error:"API-метод не найден"},traceId);return;}serveStatic(res,url.pathname);}catch(error){const code=error?.code||error?.message;const errorEntry={traceId,method:req.method,url:req.url,code:String(code||"UNKNOWN"),message:String(error?.message||error).slice(0,500),durationMs:Date.now()-started,time:new Date().toISOString()};recentServerErrors.push(errorEntry);if(recentServerErrors.length>100)recentServerErrors.splice(0,recentServerErrors.length-100);console.error(JSON.stringify({level:"error",...errorEntry}));if(!res.headersSent){if(code==="BODY_TOO_LARGE")sendJson(res,413,{ok:false,code,error:`JSON-запрос превышает лимит ${MAX_JSON_BODY_BYTES} байт`},traceId);else if(code==="INVALID_JSON")sendJson(res,400,{ok:false,code,error:"Некорректный JSON"},traceId);else sendJson(res,500,{ok:false,error:"Внутренняя ошибка сервера"},traceId);}else res.end();}});}
