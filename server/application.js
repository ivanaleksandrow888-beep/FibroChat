...
async function handleApi(req,res,pathname,searchParams){
  ...
  if(pathname==="/api/admin/invites"&&req.method==="POST"){
    ...
    const role=["user","admin"].includes(String(body.role||""))?String(body.role):"user";if(role==="admin"&&auth.user.role!=="super_admin")return sendJson(res,403,{ok:false,error:"Только головной администратор может создавать admin-инвайты"}),true;const invite={id:crypto.randomUUID(),code:`FIBRO-${crypto.randomBytes(4).toString("hex").toUpperCase()}`,role,createdBy:auth.user.id,createdAt:new Date().toISOString(),expiresAt:new Date(Date.now()+validDays*86400000).toISOString(),usedAt:null,revokedAt:null};
    const invites=readJson(INVITES_FILE);invites.push(invite);writeJson(INVITES_FILE,invites);audit("INVITE_CREATED",auth.user.id,invite.id,{validDays,role});sendJson(res,201,{ok:true,invite});return true;
  }
