...
function requireActive(req,res){const auth=requireAuth(req,res);if(!auth)return null;if(auth.user.status==="suspended"&&auth.user.suspendedUntil&&new Date(auth.user.suspendedUntil).getTime()<=Date.now()){const users=readJson(USERS_FILE);const user=users.find(x=>x.id===auth.user.id);if(user){user.status="active";user.suspendedUntil=null;writeJson(USERS_FILE,users);auth.user=user;audit("TEMP_SUSPENSION_EXPIRED",user.id,user.id,{})}}const expired=auth.user.subscriptionEndsAt&&new Date(auth.user.subscriptionEndsAt).getTime()<=Date.now();if(auth.user.status!=="active"||expired){sendJson(res,403,{ok:false,error:expired?"Срок подписки истёк":auth.user.status==="suspended"?"Аккаунт временно приостановлен":"Аккаунт ещё не подтверждён"});return null;}return auth;}
