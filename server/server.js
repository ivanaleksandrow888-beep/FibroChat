...
const PROTOCOL_CAPABILITIES = ["auth.session.v1","identity.keys.v1","messages.secure-envelope.v1","messages.delivery-queue.v1","events.sse-envelope.v1","subscriptions.v1","devices.trust.v1","network.profile.v1"];
...
function ensureDataFiles() {
  ...
  for (const filePath of [USERS_FILE, INVITES_FILE, MESSAGES_FILE, AUDIT_FILE, NOTIFICATIONS_FILE, SUPPORT_FILE, DEVICES_FILE, SESSIONS_FILE]) {
    if (!fs.existsSync(filePath)) fs.writeFileSync(filePath, "[]\n", "utf8");
  }
  if (!fs.existsSync(NETWORK_FILE)) {
    fs.writeFileSync(NETWORK_FILE, `${JSON.stringify({ nodeId: crypto.randomUUID(), networkId: `fibro-${crypto.randomBytes(4).toString("hex")}`, protocolVersion: "1.0", createdAt: new Date().toISOString(), headUserId: null, networkName: "FibroChat Network", publicBaseUrl: "" }, null, 2)}\n`, "utf8");
  }
  const network = readObject(NETWORK_FILE, {});
  let networkChanged = false;
  if (!network.nodeId) { network.nodeId = crypto.randomUUID(); networkChanged = true; }
  if (!network.networkId) { network.networkId = `fibro-${crypto.randomBytes(4).toString("hex")}`; networkChanged = true; }
  if (!network.protocolVersion || network.protocolVersion !== PROTOCOL_VERSION) { network.protocolVersion = PROTOCOL_VERSION; networkChanged = true; }
...
function readJson(filePath) { try { const x=JSON.parse(fs.readFileSync(filePath,"utf8")); return Array.isArray(x)?x:[]; } catch { return []; } }
function readObject(filePath, fallback = {}) { try { const x = JSON.parse(fs.readFileSync(filePath, "utf8")); return x && typeof x === "object" && !Array.isArray(x) ? x : fallback; } catch { return fallback; } }
function writeObject(filePath, value) { const temp=`${filePath}.tmp`; fs.writeFileSync(temp,`${JSON.stringify(value,null,2)}\n`,`utf8`); fs.renameSync(temp,filePath); }
function writeJson(filePath, value) { const temp=`${filePath}.tmp`; fs.writeFileSync(temp,`${JSON.stringify(value,null,2)}\n`,`utf8`); fs.renameSync(temp,filePath); }
...
async function handleApi(req,res,pathname,searchParams){
  ...
  if(pathname==="/api/register"&&req.method==="POST"){
    ...
    let role="user",status="pending";if(inviteCode===OWNER_INVITE&&users.length===0){role="super_admin";status="active";}else{const invites=readJson(INVITES_FILE);const invite=invites.find(x=>x.code===inviteCode&&!x.usedAt&&new Date(x.expiresAt).getTime()>Date.now());if(!invite)return sendJson(res,400,{ok:false,error:"Инвайт недействителен или уже использован"}),true;role=invite.role||"user";invite.usedAt=new Date().toISOString();invite.usedByNickname=nickname;writeJson(INVITES_FILE,invites);}
    const credentials=hashPassword(password);const now=new Date();const user={id:crypto.randomUUID(),nickname,passwordSalt:credentials.salt,passwordHash:credentials.hash,role,status,createdAt:now.toISOString(),approvedAt:status==="active"?now.toISOString():null,approvedBy:status==="active"?"self-bootstrap":null,subscriptionEndsAt:status==="active"?new Date(now.getTime()+SUBSCRIPTION_DAYS*86400000).toISOString():null,encryptionPublicKey:body.encryptionPublicKey,signingPublicKey:body.signingPublicKey,keyCreatedAt:now.toISOString()};users.push(user);writeJson(USERS_FILE,users);if(role==="super_admin"){const network=readObject(NETWORK_FILE,{});network.headUserId=user.id;network.headNickname=user.nickname;network.activatedAt=now.toISOString();writeObject(NETWORK_FILE,network);}const device=createTrustedDevice(user.id,deviceId,deviceName,"registration");audit("USER_REGISTERED",user.id,user.id,{role,status});audit("DEVICE_TRUSTED",user.id,device.id,{name:device.name,source:"registration"});const session=createSession(user.id,device.id);sendJson(res,201,{ok:true,...session,user:publicUser(user),device:publicDevice(device,device.id)});return true;
  }
  if(pathname==="/api/login"&&req.method==="POST"){
    ...
    else if(!device){const now=new Date().toISOString();device={id:deviceId,userId:user.id,name:deviceName,status:"pending",createdAt:now,approvedAt:null,approvedBy:null,lastSeenAt:now,revokedAt:null};devices.push(device);writeJson(DEVICES_FILE,devices);notify(user.id,"DEVICE_APPROVAL_REQUIRED","Новое устройство ожидает подтверждения",`Попытка входа с устройства «${device.name}». Подтвердите его на уже доверенном устройстве.`,{deviceId:device.id});audit("DEVICE_PENDING",user.id,device.id,{name:device.name});return sendJson(res,403,{ok:false,code:"DEVICE_APPROVAL_REQUIRED",error:"Новое устройство ожидает подтверждения. Откройте FibroChat на уже доверенном устройстве и подтвердите вход."}),true;}
...
server.listen(PORT,"0.0.0.0",()=>console.log(`FibroChat v0.2.0 Federation Core работает: http://localhost:${PORT}`));
