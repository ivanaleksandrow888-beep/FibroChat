...
const CLIENT_VERSION = "0.7.0-alpha7.4.1";
...
async function playVoice(button,attachment){try{if(button.dataset.audioUrl){const audio=new Audio(button.dataset.audioUrl);audio.play();return;}button.disabled=true;button.textContent="…";const blob=await fetchDecryptedAttachment(attachment);const url=URL.createObjectURL(blob);button.dataset.audioUrl=url;const audio=new Audio(url);audio.addEventListener("ended",()=>{button.textContent="▶";});button.textContent="❚❚";await audio.play();}catch(error){setChatError(error.message);button.textContent="▶";}finally{button.disabled=false;}}
